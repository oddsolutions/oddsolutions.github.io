...
We strongly suspected (and later confirmed) that the bug lied in `append_uint32_le()` within u-boot. Effectively, instead of ensuring that we fit within the allocated buffer size:

```shell
current_length + 4 <= allocated_buffer_size
```

Instead u-boot unconditionally allocated it without any sanity checks:

```shell
current_length + 4
```

This leads to a case where when the `CS#` pin is pulled high at strategic times in the boot sequence, we generate a lot of random garbage/memory corruption that reliably lands in a specific region of memory that inneviatbly overflows the allocated buffer and crashes u-boot, which falls back to waiting for commands via u-boot shell.
...
A collection of all our knowledge on this device, as well as resources to utilize this chain of exploits can be found [here](https://github.com/oddsolutions/korlan-secure-boot-bypass/).
...
- We have a few issues with this statement:
  ...
  * We disagree with the nature of the chain of vulnerabilities being designated as `a previously reported exploit`. This is not eMMC fault injection in u-boot by shorting an eMMC data-line pin, like as we reported on boreal.
  ...
  - It was rated a `5.1`, which does not seem appropriate given this chain results in persistent secure-boot bypass, the impact is far higher than what was associated to the CVE in our opinion.
  - Normally we'd never discuss bug-bounty amount, but the amount associated with this was _far_ below what would typically be expected of this classification of vulnerability in our opinion, even when compared to `CVE-2023-48424` which we reported previously.

**Ultimately we respect the decisions, and are very grateful that Google supported our research of their devices and sponsored us to attend this wonderful conference. We look forward to working on Google devices in the future but do hope that expectations and that the ratings and criticality matrices is more clearly defined in the future.**
...
- 21-MAY-2024 - We follow up stating that we plan to release the writeup and resources on 30-JUN-2024, as the vulnerability had surpassed the original disclosure timeline. We received a response that the issue still being a work in progress.
...
- 25-JUL-2025 - The vulnerability patch was deployed via an OTA update delivered through the Pixel Tablet.
- 05-DEC-2025 (YES, 202_5_) - `CVE-2025-36938` is assigned and released as part of the [December 2025 Android Security Bulletin (ASB)](https://source.android.com/docs/security/bulletin/pixel/2025-12-01). It is rated as a 5.1.

## CVE Tracking

[CVE-2025-36938](https://source.android.com/docs/security/bulletin/pixel/2025-12-01#pixel) - NAND Fault Injection - Labeled simply as "Pixel Tablet Dock"
